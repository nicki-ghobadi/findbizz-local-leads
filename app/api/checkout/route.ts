...
export async function POST(req: NextRequest) {
  try {
    ...
    const priceId = requireEnv("STRIPE_PRICE_CAD_59");
    ...
    const industryLabel = (payload.industryLabel as string | undefined) || industry;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${requireEnv("NEXT_PUBLIC_SITE_URL")}/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${requireEnv("NEXT_PUBLIC_SITE_URL")}/`,
    });
